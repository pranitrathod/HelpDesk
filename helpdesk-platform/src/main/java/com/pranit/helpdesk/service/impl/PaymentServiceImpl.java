...
public class PaymentServiceImpl implements PaymentService {
  ...
  public PaymentServiceImpl(PaymentTransactionRepository transactions,
      PaymentGatewayRegistry gateways, ObjectMapper objectMapper) {
  ...
  @Override
  public PaymentResponse createCheckout(String idempotencyKey, CreatePaymentRequest request) {
    return transactions.findByIdempotencyKey(idempotencyKey).map(this::response).orElseGet(() -> {
      PaymentTransaction transaction =
          transactions.save(new PaymentTransaction(request.getMerchantReference(), idempotencyKey,
  ...
  public void processWebhook(PaymentGateway gateway, String payload, String signature) {
    ...
    try {
      ...
      String providerOrderId =
          event.at("/payload/payment/entity/order_id").asText(event.path("order_id").asText());
      if (providerOrderId.isBlank()) {
        throw new PaymentGatewayException("Payment webhook does not contain an order id", null);
      }
      PaymentTransaction transaction =
      ...
      String eventName = event.path("event").asText(event.path("status").asText());
      if (eventName.contains("captured") || eventName.equalsIgnoreCase("CHARGED"))
        transaction.markSucceeded();
      if (eventName.contains("failed") || eventName.equalsIgnoreCase("FAILED"))
        transaction.markFailed();
    } catch (PaymentGatewayException exception) {
      throw exception;
    } catch (Exception exception) {
      throw new PaymentGatewayException("Unable to process payment webhook", exception);
    }
  }

  private PaymentResponse response(PaymentTransaction transaction) {
    return new PaymentResponse(transaction.getId(), transaction.getMerchantReference(),
        transaction.getProviderOrderId(), transaction.getCheckoutToken(), transaction.getAmount(),
        transaction.getCurrency(), transaction.getGateway(), transaction.getStatus());
  }
}
