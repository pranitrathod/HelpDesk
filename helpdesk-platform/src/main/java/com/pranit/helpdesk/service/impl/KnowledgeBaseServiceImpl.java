...
public class KnowledgeBaseServiceImpl implements KnowledgeBaseService {
  ...
  public ArticleResponse publish(CreateArticleRequest request) {
    ...
    return response(repository.save(new KnowledgeArticle(request.getSlug(), request.getTitle(),
        request.getSummary(), request.getContent(), request.isPublished())));
  }
  @Transactional(readOnly = true)
  public List<ArticleResponse> listPublished() {
    return repository.findByPublishedTrueOrderByUpdatedAtDesc()
        .stream()
        .map(this::response)
        .toList();
  }
  ...
  private KnowledgeArticle article(String slug) {
    return repository.findBySlugAndPublishedTrue(slug).orElseThrow(
        () -> new ResourceNotFoundException("Published article", slug));
  }
  private ArticleResponse response(KnowledgeArticle article) {
    return new ArticleResponse(article.getId(), article.getSlug(), article.getTitle(),
        article.getSummary(), article.getContent(), article.isPublished(),
        article.getHelpfulVotes(), article.getUpdatedAt());
  }
}
