...
public class CustomerFeedbackServiceImpl implements CustomerFeedbackService {
  ...
  public FeedbackResponse submit(Long customerId, SubmitFeedbackRequest request) {
    ...
    return response(repository.save(new CustomerFeedback(
        request.getTicketId(), customerId, request.getRating(), request.getComment())));
  }
  @Transactional(readOnly = true)
  public List<FeedbackResponse> findForTicket(Long ticketId) {
    return repository.findByTicketId(ticketId).stream().map(this::response).toList();
  }
  private FeedbackResponse response(CustomerFeedback feedback) {
    return new FeedbackResponse(feedback.getId(), feedback.getTicketId(), feedback.getRating(),
        feedback.getComment(), feedback.getSubmittedAt());
  }
}
